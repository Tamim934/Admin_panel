...
import { Button, Dialog, DialogActions, DialogContent, DialogTitle, TextField, Grid, Card, CardMedia, CardContent, Typography, Switch, FormControlLabel, Select, MenuItem, Input, Box, Fab, FormControl, InputLabel } from '@mui/material';
...
const Products = () => {
  ...
  const addProducts = async (product) => {
    try {
      ...
      const { data } = await axios.post(api, {...product, media}, {
        headers: {
...
getsubcategories()
  ...
  return (
    ...
    <Typography variant="body1" gutterBottom style={{ textAlign: 'center' }}>
      Welcome to our products page, a cornucopia of items waiting to be discovered. From the latest trends to timeless classics, there's something for everyone. Start your shopping adventure here!
    </Typography>
  </div>
...
/>
  ...
  <DialogContent>
    <Grid container spacing={3}>
        ...
        <FormControl variant="outlined" fullWidth>
          ...
          >
            {Categories.map((e, index) => (
              <MenuItem key={index} value={index}>{e.name}</MenuItem>
            ))}
          </Select>
        </FormControl>
      </Grid>
        ...
        <FormControl variant="outlined" fullWidth>
          ...
          >
            {Brands.map((brand, index) => (
              <MenuItem key={index} value={index}>{brand.name}</MenuItem>
            ))}
          </Select>
        </FormControl>
      </Grid>
      ...
      <Grid item xs={12}>
        <FormControlLabel
          control={<Switch checked={formik.values.hasDiscount} onChange={formik.handleChange} name="hasDiscount" />}
          label="Has Discount"
        />
      </Grid>
      <Grid item xs={12}>
        <FormControlLabel
          control={<Switch checked={formik.values.isNew} onChange={formik.handleChange} name="isNew" />}
          label="Is New"
        />
      </Grid>
      <Grid item xs={12}>
        <ReactQuill theme="snow" value={formik.values.description} onChange={value => formik.setFieldValue('description', value)} />
      </Grid>
    </Grid>
  </DialogContent>
  <DialogActions>
