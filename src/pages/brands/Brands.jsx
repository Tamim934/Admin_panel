...
getBrands()
  ...
  const handleSubmit = async () => {
    try {
      ...
      const { data } = await axios.post(brandsAPI, {...newBrand, img: fileData.img}, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      })
  ...
  async function deleteBrand(id){
    try {
      const {data}=await axios.delete(`${brandsAPI}/${id}` ,{
        headers: {
  ...
  const handleUpdate = async () => {
    try {
      ...
      formData.append('file', imgFile)
      const fileData = await singleFile(formData)
      const { data } = await axios.put(`${brandsAPI}/${editBrand.id}`, {...editBrand, img: fileData.img}, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      })
      setBrands(Brands.map(brand => brand.id === data.id ? data : brand))
      handleCloseEdit()
    } catch (error) {
      console.log(error)
    }
...
<div className='mt-[20px]'>
  <Typography variant="h3" component="h2" gutterBottom style={{ color: '#3f51b5', textAlign: 'center' }}>
    🌟 Embrace the World of Brands 🌟
  </Typography>
  <Typography variant="body1" gutterBottom style={{ textAlign: 'center' }}>
    Step into our realm where each brand tells a unique story. Discover their essence, shop their offerings, and immerse yourself in the rich tapestry of narratives we've woven together for you.
  </Typography>
</div>
  ...
  <DialogContent>
    <TextField
      ...
      type="text"
      fullWidth
      value={editBrand ? editBrand.name : ''}
        ...
        {Brands.filter(brand => brand.name.toLowerCase().includes(searchTerm.toLowerCase())).map((brand) => (
            <Grid item xs={12} sm={6} md={3} key={brand.id}>
              <Card style={{ height: '350px', width: '350px', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', backgroundColor: '#f5f5f5', borderRadius: '15px', boxShadow: '0px 10px 15px -3px rgba(0,0,0,0.1), 0px 4px 6px -2px rgba(0,0,0,0.05)' }}>
                ...
                </CardContent>      <div className='flex gap-[10px]'> <button 
  ...
  <button 
  className='bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded text-center ml-[0px]' 
